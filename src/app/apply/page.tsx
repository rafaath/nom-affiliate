...
import {
  claimPendingPartnerApplication,
...
export default async function ApplyPage({
  ...
  if (currentUser?.email) {
    try {
      ...
      const recoveredProfile = await claimPendingPartnerApplication(
        currentUser.id,
        currentUser.email,
      );
    ...
    <div className="min-h-screen bg-paper">
      ...
      <main className="marketing-container grid gap-10 py-12 lg:grid-cols-12 lg:py-20">
        <section className="lg:col-span-5">
          ...
          <h1 className="marketing-title mt-6 max-w-[10ch]">
            Tell us how you can help restaurants succeed.
          </h1>
          <p className="mt-6 max-w-lg leading-8 text-ink-body">
            No existing network is required. Tell us how you plan to find
            interested restaurants, then follow the review in your portal.
          </p>
          <Card className="mt-9 bg-lilac">
            <CardHeader>
              <CardTitle>After applying</CardTitle>
              <CardDescription>
                Your account and application stay separate from approval.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-3 text-sm text-ink-body">
              <p>01 · Verify your account with Google</p>
              <p>02 · Nom reviews the application</p>
              <p>03 · Lead submission unlocks only after approval</p>
              <p>
                04 · Existing portal history stays visible if access changes
              </p>
            </CardContent>
          </Card>
        </section>
          ...
          <CardHeader>
            <CardTitle>
              {isFinishingApplication
                ? "Finish your Nom Partner application"
                : "Apply to become a Nom Partner"}
            </CardTitle>
            <CardDescription>
              {isFinishingApplication ? (
                <>
                  You are logged in as {currentUser?.email}. Submit this form to
              ...
              <div className="grid gap-5">
                <div className="rounded-lg border border-plum/15 bg-lilac/45 p-5">
                  ...
                  <p className="mt-3 text-sm leading-7 text-ink-body">
                    Google verifies your email securely. Nom never receives your
                    Google password, and signing in does not approve your
                    partner application.
                  </p>
                </div>
                <GoogleSignInButton
              ...
              <>
                ...
                >
                  ...
                  <div className="grid gap-5 md:grid-cols-2">
                    ...
                    <Field
                      label="Locality / areas covered"
                      name="localityAreasCsv"
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="partnerType">
                      How do you want to work with Nom?
                    </Label>
                    <NativeSelect
                      id="partnerType"
                      name="partnerType"
                  ...
                  <Field
                    label="Restaurant contacts you already have (0 is okay)"
                    name="restaurantNetworkSize"
                    type="number"
                    min={0}
                    defaultValue={0}
                    required
                  ...
                  <p className="-mt-2 text-sm text-muted-foreground">
                    If you share a resume, make sure anyone with the link can
                    view it.
                  </p>
                  <TextField
                    label="Restaurant experience (write ‘none’ if you’re new)"
                    name="restaurantExperience"
                    required
                  />
                  <TextField
                    label="Current work / background"
                    name="background"
                    required
                  />
                  <div className="grid gap-5 md:grid-cols-2">
                  ...
                  <div className="rounded-lg border border-plum/15 bg-lilac/45 p-4">
                    <input
                      type="hidden"
                      name="applicationTermsVersion"
                      value={APPLICATION_TERMS_VERSION}
                    />
                    <label
                      className="flex items-start gap-3 text-sm leading-6"
                      htmlFor="applicationTermsAccepted"
                    >
                      <input
                      ...
                      />
                      <span>
                        I confirm that I am at least 18 and that the information
                        above is accurate. I accept the{" "}
                        <Link
                        ...
                        . Terms version {APPLICATION_TERMS_VERSION}; privacy
