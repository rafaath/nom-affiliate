...
import { getSafeAuthRedirectPath, validatePasswordReset } from '@/lib/supabase/auth-flow';
import { createSupabaseServerClient } from '@/lib/supabase/server';

function authErrorRedirect(path: '/forgot-password' | '/reset-password', message: string): never {
  redirect(`${path}?error=${encodeURIComponent(message)}` as never);
}

function getPasswordRecoveryCallbackUrl() {
  const applicationUrl = new URL(process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3021');
...
export async function requestPasswordResetAction(formData: FormData) {
  ...
  if (!email) {
    authErrorRedirect('/forgot-password', 'Enter the email address for your account.');
  }
...
export async function updatePasswordAction(formData: FormData) {
  ...
  if (userError || !data.user) {
    authErrorRedirect('/forgot-password', 'This password-reset link is invalid or expired. Request a new one.');
  }
